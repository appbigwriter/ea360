  ...
  async generateComponent(componentType, options = {}) {
    ...
    try {
      ...
      if (stats.commands !== undefined) {
        console.log(
          chalk.gray(`   Commands: ${stats.commands} | Dependencies: ${stats.dependencies}`)
        );
      }
      ...
      await this.transactionManager.recordOperation(transactionId, {
        ...
        metadata: {
          componentType,
          componentId: variables.AGENT_NAME || variables.TASK_ID || variables.WORKFLOW_ID,
        },
      });
      ...
      if (!options.skipManifest) {
        const componentInfo = {
          ...
          description:
            variables.WHEN_TO_USE || variables.TASK_DESCRIPTION || variables.WORKFLOW_DESCRIPTION,
        };
    ...
    for (const taskInfo of tasksToCreate) {
      try {
        ...
        await this.elicitationEngine.mockSession({
          ...
          workflow: ["Parse command parameters", "Execute main logic", "Return results"],
