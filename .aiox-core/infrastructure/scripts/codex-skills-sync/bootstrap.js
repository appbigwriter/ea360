...
function parseAgentFile(filePath, yaml) {
  ...
  const title =
    String(agent.title || "").trim() || findScalar(yamlBlock, ["title"]) || basenameNoExt(filePath);
...
function main() {
  ...
  if (!plans.length) {
    throw new Error(
      "No skills were discovered. Run this from the project root that contains .aiox-core/ and squads/."
    );
  }
  ...
  if (!args.quiet) {
    ...
    console.log("");
    console.log(
      `Discovered:   ${corePlans.length} core skills, ${squadResult.plans.length} squad entry skills`
    );
    console.log(
      `Results:      ${Object.keys(summary)
    ...
    if (results.some((item) => item.status === "skipped_existing")) {
      console.log("");
      console.log(
        "Some existing non-generated skills were skipped. Re-run with --force only if you want backups and overwrite."
      );
    }
    ...
    console.log(
      "- Open or restart Codex CLI from this project root if the $ menu does not refresh immediately."
    );
