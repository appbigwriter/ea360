  ...
  for (const item of resolved.expected) {
    ...
    if (canRoundtripSkillPayload) {
      ...
      if (!expectedIds.has(target)) {
        errors.push(
          `self-test Skill payload target is not a generated skill: ${target || "<empty>"}`
        );
      } else if (target !== item.skillId) {
        errors.push(
          `self-test Skill payload target mismatch: expected "${item.skillId}", got "${target}"`
        );
      }
    }
...
function getThinLegacyRedirectIssues(content, canonicalSkillId, legacySkillId) {
  ...
  if (
    !lines.some((line) =>
      new RegExp(`^This legacy alias redirects to canonical skill \`${canonical}\`\\.$`).test(line)
    )
  ) {
  ...
  const invalidLines = lines.filter(
    (line) => !allowedLinePatterns.some((pattern) => pattern.test(line))
  );
...
function validateCodexSkills(options = {}) {
  ...
  const expectedByCanonicalPath = new Map(
    expected.map((item) => [`.aiox-core/development/agents/${item.filename}`, item])
  );
  const orphaned = [];
  const legacy = [];
  const legacyAliases = [];
  ...
  if (resolved.strict) {
    const dirs = fs
      ...
      .filter(
        (entry) =>
          entry.isDirectory() && (entry.name.startsWith("aiox-") || entry.name.startsWith("aios-"))
      )
    ...
    for (const dir of dirs) {
      if (legacyIds.has(dir)) {
        ...
        const relativeSkillPath = path.join(
          path.relative(resolved.projectRoot, resolved.skillsDir),
          dir,
          "SKILL.md"
        );
        const classification = classifyLegacySkillAlias(
      ...
      if (dir.startsWith("aiox-") && !expectedIds.has(dir)) {
        ...
        const canonicalAgentPath = extractCanonicalAgentPath(content);
        const duplicateOf = canonicalAgentPath
          ? expectedByCanonicalPath.get(canonicalAgentPath)
          : null;
...
function formatHumanReport(result) {
  if (result.ok) {
    const suffix =
      result.selfTests && result.selfTests.length > 0
