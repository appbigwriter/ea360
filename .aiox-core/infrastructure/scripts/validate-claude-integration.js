...
function listClaudeAgentSkillIds(skillsAgentsDir) {
  ...
  return fs
    .readdirSync(skillsAgentsDir, { withFileTypes: true })
    .filter(
      (entry) =>
        entry.isDirectory() && fs.existsSync(path.join(skillsAgentsDir, entry.name, "SKILL.md"))
    )
...
function validateClaudeIntegration(options = {}) {
  ...
  if (sourceAgents.length > 0 && skillAgents.length !== sourceAgents.length) {
    errors.push(
      `Claude agent skill count differs from source (${skillAgents.length}/${sourceAgents.length})`
    );
  }
  ...
  if (sourceAgents.length > 0 && commandAgents.length !== sourceAgents.length) {
    warnings.push(
      `Legacy Claude command count differs from source (${commandAgents.length}/${sourceAgents.length})`
    );
  }
