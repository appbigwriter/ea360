...
function transform(agentData) {
  ...
  const whenToUse =
    rawAgent.whenToUse || fallbackAgent.whenToUse || `Use for ${title.toLowerCase()} tasks`;
...
function buildProtocolSection(yaml) {
  ...
  if (yaml.evidence_policy) {
    ...
    if (ep.accepted_types && Array.isArray(ep.accepted_types))
      text += `- Accepted types: ${ep.accepted_types.join(", ")}\n`;
    if (ep.reject_if && Array.isArray(ep.reject_if))
      text += `- Reject if: ${ep.reject_if.map((r) => `"${r}"`).join(", ")}\n`;
    items.push(text + "\n");
  }
...
function normalizeCommands(commands) {
  ...
  return commandList.map(normalizeCommand).filter((cmd) => cmd && cmd.name);
}
...
function buildDescription(id, name, title, whenToUse) {
  let desc = whenToUse.replace(/\n/g, " ").replace(/\s+/g, " ").trim();

  if (desc.length > 300) {
    desc = desc.substring(0, 297) + "...";
  }

  const triggerPhrases = [`activate ${id}`, `switch to ${id}`, `@${id}`];
...
function sanitizeGeneratedMarkdown(content) {
  return addLanguageToUntypedFences(content).split("\n").map(sanitizeBareStarCommands).join("\n");
}
