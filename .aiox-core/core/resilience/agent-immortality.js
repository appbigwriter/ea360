...
function normalizeState(state = {}) {
  ...
  return {
    ...
    criticalVariables:
      state.criticalVariables && typeof state.criticalVariables === "object"
...
function buildLegacy(state, cause, options = {}) {
  ...
  const memoryTail = normalized.workingMemory.slice(
    Math.max(0, normalized.workingMemory.length - tailSize)
  );
  ...
  const summary = [
    ...
    legacy.diff
      ? `Delta: ${truncate(legacy.diff, options.maxLegacyChars ?? DEFAULT_CONFIG.maxLegacyChars)}`
      : "",
  ]
    .filter(Boolean)
    .join("\n");
...
function generateDirectives(cause, error, state = {}) {
  ...
  switch (cause) {
    ...
    case CauseOfDeath.TOOL_EXECUTION_FAILURE:
      ...
      directives.push(
        `Treat the previous tool failure as toxic until parameters change: ${truncate(normalizedError.message, 160)}`
      );
      break;
    case CauseOfDeath.RECURSIVE_LOOP:
...
class AgentImmortalityProtocol extends EventEmitter {
  constructor(projectRoot = process.cwd(), options = {}) {
    ...
    this.stateCommitLog = options.stateCommitLog || new StateCommitLog(projectRoot, this.config);
    this.autopsyEngine =
      options.autopsyEngine ||
      new AutopsyEngine(projectRoot, {
        ...this.config,
        stateCommitLog: this.stateCommitLog,
      });
    this.reincarnationQueue =
      options.reincarnationQueue || new ReincarnationQueue(projectRoot, this.config);
