...
class HierarchicalContextManager extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    this.minRecentMessages =
      Number.isInteger(options.minRecentMessages) && options.minRecentMessages >= 0
  ...
  async _fitLongTermSummariesToBudget() {
    ...
    if (this._getTotalTokens() > this.maxTokens && this._longTermSummaries.length > 0) {
      this._longTermSummaries = [this._truncateSummaryMessage(this._longTermSummaries[0])];
    }
  }
  ...
  async _summarizeMessages(messages, options) {
    ...
    const summaryContent =
      this._normalizeSummaryResult(rawSummary) ||
  ...
  _truncateTextToTokenBudget(text, tokenBudget) {
    ...
    while (
      this._countTextTokens(result) > tokenBudget &&
