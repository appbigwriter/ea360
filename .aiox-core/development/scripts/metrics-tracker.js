...
class MetricsTracker {
  ...
  generateInsights(metrics) {
    ...
    const peakHour = Object.entries(hourlyDist).sort(([, a], [, b]) => b - a)[0];
  ...
  calculatePerformanceMetrics(_improvements) {
    ...
    return {
      ...
      fastest_improvement:
        successful.filter((i) => i.duration_ms).sort((a, b) => a.duration_ms - b.duration_ms)[0]
          ?.duration_ms || null,
      slowest_improvement:
        successful.filter((i) => i.duration_ms).sort((a, b) => b.duration_ms - a.duration_ms)[0]
          ?.duration_ms || null,
    };
  }
