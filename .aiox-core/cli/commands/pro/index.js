...
async function setupAction(options) {
  ...
  if (options.verify) {
    ...
    try {
      ...
      const found =
        fs.existsSync(manifestPath) || fs.existsSync(versionPath) || fs.existsSync(packagePath);
      if (found) {
...
async function updateAction(options) {
  ...
  if (options.check || options.dryRun) {
    ...
    if (options.dryRun) {
      ...
      console.log(
        "  Actions:      validate license, download signed artifact, verify sha256, install package, scaffold assets"
      );
      console.log("");
      return;
    }
  ...
  if (options.includeCore) {
    console.log(
      "Note: --include-core is handled by updating @aiox-squads/core first, then rerunning aiox pro update."
    );
  }
